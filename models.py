...
class DecoderRNN(nn.Module):
    ...
    def forward(self, encoder_outputs, encoder_hidden, max_length=15):
        ...
        decoder_input = torch.empty(batch_size, 1, dtype=torch.long, device=device).fill_(self.vocab['<bos>'])
        decoder_outputs = []
...
class AttnDecoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size, te_vocab, dropout_p=0.1):
